...
# IMAGEJPATH = "/Applications/Fiji.app/Contents/MacOS"
...
def download_rendered_planes(image, tiff_stack_dir, region=None):
    """
    Download the specified image as a Z-stack of 'rendered' RGB tiffs to local directory.
    NB: The pixel values in the downloaded tiff are NOT the raw data as in OMERO.
...
def download_raw_planes(image, tiff_stack_dir, cIndex, region=None):
    """
    Download the specified image as a Z-stack of 'raw' tiffs to local directory.
    ...
    if region is not None:
        ...
        planes = image.getPrimaryPixels().getTiles(zctTileList)    # A generator (not all planes in hand)
    else:
...
def do_processing(tiff_stack_dir, destination, sizeZ, axis="Y"):
    """
...
run("Image Sequence...", "open=&ippath number=%s starting=1 increment=1 scale=100 file=[] or=[] sort");
run("3D Project...", "projection=[Brightest Point] axis=%s-Axis slice=1 initial=0 total=360 rotation=10 lower=1 upper=255 opacity=0 surface=100 interior=50");
run("Image Sequence... ", "format=JPEG name=["+opname+"] start=0 digits=4 save="+oppath );
    ...
    macro_args = "*".join( [tiff_stack_dir, destination])        # can't use ";" on Mac / Linu. Use "*"
...
def process_image(conn, image, tiff_stack_dir, processed_img_dir, axis, useRawData=False, cIndex=0, region=None):
    ...
    Download the Z-planes of the input image as separate tiffs into tiff_stack_dir/
    call the processing step (output images should be put into processed_img_dir/ )
    and then create a new Image in OMERO from these images
    """
    ...
    # Depending on what our processing needs, use rendered data or raw pixels as tiffs
    if useRawData:
    ...
    do_processing(tiff_stack_dir, processed_img_dir, sizeZ, axis)
...
#     current_dir = os.getcwd()
#     tiff_stack_dir = os.path.join(current_dir, "tiff_stack/")
#     print tiff_stack_dir
#     processed_img_dir = os.path.join(current_dir, "processed_img/")
#     try:
#         os.mkdir(processed_img_dir)
#     except:
#         pass
#     do_processing(tiff_stack_dir, processed_img_dir, str(20), axis="Y")
